...
for (year in unique(dt$ANNEE)) {
      ...
      fig <- fig %>% add_trace(
                     x= dt[[feature]][dt$ANNEE == year], y= dt[['EXPO']][dt$ANNEE == year], type = 'bar', name = paste0('Exposition_', year),
                     hovertemplate = 'Expo: %{y:.0f}<br>'
                  )
      fig<- fig %>% add_trace(
                     x= dt[[feature]][dt$ANNEE == year], y= dt[['Freq']][dt$ANNEE == year], type = 'scatter', mode = 'lines', yaxis = 'y2', name= paste0('Fréquence_', year),
...
plot_obs_2(df, feature, use_year = F)
plot_obs_2(df, feature, use_year = T)
